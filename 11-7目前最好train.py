...
from pseudo_ood_utils import make_pseudo_ood, compose_tiered_ood
...
def train_and_eval(cfg: TrainConfig):
    ...
    # ---- 构造 ramp（任选其一）----
    # 1) 分段版（和 head-warmup 更贴合）
    # ramp_coef = build_piecewise_ramp(cfg.num_epochs, warmup_head_epochs=cfg.warmup_head_epochs)
    ...
    for epoch in range(cfg.num_epochs):
        ...
        # ===== Warmup 保护：前 warmup_head_epochs 轮只训分类头 =====
        # 说明：warmup 阶段不再把 OOD 系数清零；由 ramp_coef(epoch) 控制实际强度
        if epoch < cfg.warmup_head_epochs:
            ...
            use_ood = True
            use_lid = True
            if epoch < (cfg.warmup_head_epochs + 2):
                eff_T = 1.2
            else:
                eff_T = min(cfg.T, 1.5)
        # ==========================================================
        ...
        for inputs, targets in tqdm(
            ...
            ##############################
            # ====== 使用 Composer 统一生成 OOD（t1/t2/t3/t0）======
            if use_ood:
                # Tier-3 需要的两个回调（避免在 utils 里耦合 backbone）
            ...
            #######################################
            # —— 无论是否开启 LID，都记录“只用于日志”的 ID/OOD 能量 ——
            # ID：直接用 logits_li 计算一份能量用于日志（不影响训练梯度）
            ...
            # 如需看真 OOD 的能量均值，也可另开一个 meter_E_real_sum 统计

            # ====== (per-batch) 原始项 ======
            L_ce = criterion(logits_id, targets)
            L_conf = confidence_penalty(logits_id, lam=cfg.lambda_conf)
            # 说明：L_lid / L_ood 已在上方根据 use_lid/use_ood 得到（可能为 0）
            ...
            # L_ood      = torch.clamp(L_ood,      max=5.0)   # 可在 3~8 之间试；先用 5.0
            # L_ood_real = torch.clamp(L_ood_real, max=5.0)

            w_ood_pseudo = (cfg.beta * max(coef, 0.15)) * L_ood

            coef_real = 1.0
            if epoch == cfg.warmup_head_epochs:
                coef_real = 0.5
            elif epoch == cfg.warmup_head_epochs + 1:
                coef_real = 0.75

            w_ood_real = coef_real * (cfg.beta_real) * L_ood_real  # 新增coef_real控制
            # w_ood_real   = (cfg.beta_real       ) * L_ood_real

            w_ood = w_ood_pseudo + w_ood_real
            w_conf = L_conf
            # L_total = w_ce + w_lid + w_ood + w_conf

            # max_ratio = 0.3  # OOD(伪+真) 不超过其余项的 70%  10.23修改
            ...
            if epoch < warm + 1:
                ...
                max_ratio = 1.00  # 训练后1/3彻底放开，让OOD项吃满梯度
            # if epoch < warm + 2:      # OOD刚开启的2轮
            #     max_ratio = 0.35
            # elif epoch < warm + 5:
            #     max_ratio = 0.45
            # else:
            ...
            # （可选）防止偶发的 OOD 爆项主导训练
            # max_ratio = 0.6
            # w_ood = torch.minimum(w_ood, max_ratio * (w_ce.detach() + 1e-6))
            ...
            raw_meters["lood"] += float(L_ood) * bs
            raw_meters["conf"] += float(L_conf) * bs
            raw_meters["lood_pseudo"] += float(L_ood) * bs
            raw_meters["lood_real"] += float(L_ood_real) * bs
            raw_meters["n"] += bs

        # ====== （批次循环结束后）按 epoch 求均值、评估、日志、保存、调度 ======
